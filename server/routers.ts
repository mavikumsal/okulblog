...
export const appRouter = router({
  ...
  admin: router({
    ...
    bulkApproveDocumentImportDrafts: adminProcedure.input(z.object({ ids: z.array(z.number().int().positive()).min(1).max(100) })).mutation(async ({ ctx, input }) => { const results: Array<{ id: number; success: boolean; error?: string; contentId?: number }> = []; for (const id of input.ids) { try { const draft = await getDocumentImportDraft(id); if (!draft) throw new Error("Taslak bulunamadı."); if (!draft.categoryId && !draft.institutionCategoryId) throw new Error("Kategori seçilmemiş."); const asset = await getMediaAsset(draft.mediaAssetId); if (!asset) throw new Error("Medya varlığı bulunamadı."); const contentId = await createContentItem({ title: draft.title, contentType: "document", summary: draft.summary ?? undefined, body: asset.publicUrl ?? "", coverImageUrl: draft.coverImageUrl, categoryId: draft.categoryId, institutionCategoryId: draft.institutionCategoryId, createdBy: ctx.user.id, status: "published" }); await createMediaAssetLink({ mediaAssetId: draft.mediaAssetId, targetType: "content", targetId: contentId, role: "document-file", createdBy: ctx.user.id }); await updateDocumentImportDraft(id, { status: "approved", reviewedBy: ctx.user.id, reviewedAt: new Date() }); results.push({ id, success: true, contentId }); } catch (error) { results.push({ id, success: false, error: error instanceof Error ? error.message : "Yayınlama başarısız." }); } } return { success: results.every(item => item.success), results }; }),
