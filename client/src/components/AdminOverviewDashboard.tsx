...
export function AdminOverviewDashboard({ userName, content, userCount, pendingCount, onNavigate }: AdminOverviewDashboardProps) {
  ...
  return <div className="space-y-6">
    ...
    <section className="grid gap-5 xl:grid-cols-[1.45fr_.85fr]">
      <article className="rounded-[22px] border border-[#e1e8e7] bg-white p-5 shadow-[0_8px_22px_rgba(25,63,89,.04)] sm:p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><BarChart3 size={17} className="text-[#2c9a9a]" /><h2 className="font-black text-[#193f59]">İçerik Performansı</h2></div><p className="mt-1 text-xs text-[#829094]">Mevcut içerik dağılımına göre genel görünüm</p></div><div className="flex rounded-lg bg-[#f2f5f4] p-1 text-[11px] font-bold text-[#71838b]"><span className="rounded-md bg-white px-2.5 py-1 text-[#193f59] shadow-sm">Günlük</span><span className="px-2.5 py-1">Haftalık</span><span className="px-2.5 py-1">Aylık</span></div></div><div className="mt-7 flex h-48 items-end gap-2 rounded-xl bg-[linear-gradient(to_bottom,transparent_49%,#edf2f0_50%,transparent_51%)] px-2 pb-1 sm:gap-4">{[38, 55, 44, 68, 58, 78, Math.min(92, 42 + publishedCount * 5)].map((height, index) => <div key={index} className="flex flex-1 flex-col items-center justify-end gap-2"><div className="w-full max-w-10 rounded-t-lg bg-gradient-to-t from-[#2c9a9a] to-[#65c8bb] transition-all" style={{ height: `${Math.round(height * 1.65)}px` }} /><span className="text-[10px] text-[#9aa7a7]">{["Pzt", "Sal", "Çar", "Per", "Cum", "Cmt", "Paz"][index]}</span></div>)}</div><div className="mt-5 flex gap-5 text-xs font-semibold text-[#73858a]"><span className="inline-flex items-center gap-2"><i className="h-2 w-2 rounded-full bg-[#2c9a9a]" /> İçerik adedi</span><span className="inline-flex items-center gap-2"><i className="h-2 w-2 rounded-full bg-[#e5a83d]" /> Etkileşim verisi</span></div></article>
