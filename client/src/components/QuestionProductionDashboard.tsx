...
function Metric({ label, value, tone }: { label: string; value: number; tone: string }) {
  return (
    <div className={`rounded-2xl p-4 ${tone}`}>
      <p className="text-[11px] font-bold uppercase tracking-[.12em] opacity-70">{label}</p>
...
export function QuestionProductionDashboard({ compact = false, className = "" }: Props) {
  ...
  return (
    <section className={`rounded-[28px] border border-[#dfe3da] bg-white p-5 shadow-[0_18px_45px_rgba(16,46,73,.07)] sm:p-6 ${className}`} aria-labelledby="question-production-title">
      ...
      {total === 0 ? (
        compact ? <div className="mt-4 flex items-center gap-2 rounded-2xl bg-[#fafbf8] px-3 py-3 text-xs font-bold text-[#718184]"><FileQuestion size={17} className="text-[#9aa8a5]" /> Henüz soru üretimi yok.</div> : <div className="mt-5 rounded-2xl border border-dashed border-[#d9ddd5] bg-[#fafbf8] p-5 text-center"><FileQuestion className="mx-auto text-[#9aa8a5]" size={25} /><p className="mt-2 text-sm font-bold text-[#52666d]">Henüz üretim kaydı bulunmuyor.</p><p className="mt-1 text-xs text-[#899598]">İlk sorunuzu oluşturduğunuzda özet burada görünecek.</p></div>
      ) : (
