...
export const quickContentTypes: ContentType[] = ["test", "video", "document", "game", "simulation"];
...
function CategoryTree({ nodes, contentByCategory }: { nodes: CategoryNode[]; contentByCategory: Map<number, number> }) {
  ...
  const renderBranch = (node: CategoryNode, depth = 0): ReactElement => {
    ...
    return <div key={node.id} className={depth ? "ml-4 border-l border-[#dbe7e2] pl-4 sm:ml-6 sm:pl-5" : ""}>
      <div role="button" tabIndex={0} onClick={() => visit(node.id)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); visit(node.id); } }} className={`flex cursor-pointer items-center gap-2 rounded-2xl border px-3 py-3 shadow-[0_8px_22px_rgba(18,48,74,.04)] transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5540e8] ${isLastVisited ? "border-[#d7ae4d] bg-[#fff9e9] ring-2 ring-[#f0d98b]/40" : "border-[#e1eae6] bg-white"}`}>
...
export default function ContentHub() {
  ...
  const baseContent = ((overview.data?.content ?? []) as ContentItem[]).filter(item => (item.status === "published" || item.status === "pending") && item.contentType === type && (isNews || item.categoryId));
  ...
  const visibleContent = isNews ? newsPagination.items : content;
  const Icon = config.icon;
  const classBreadcrumb = buildClassBreadcrumb(selectedClassNode?.name ?? null, selectedSubjectNode?.name ?? null);
  const classPageLinks = quickContentTypes.filter(contentType => contentType !== type);
  ...
  return <main className="min-h-screen bg-[#f7f6f1] text-[#12304a]">
    ...
    <section className="border-b border-[#d7e3df] bg-[#dceee9]"><div className="container py-8 sm:py-12"><div className="flex flex-wrap items-start justify-between gap-6"><div className="max-w-3xl"><nav aria-label="Breadcrumb" className="mb-4 flex flex-wrap items-center gap-2 text-xs font-semibold text-[#6c8580]">{classBreadcrumb.map((crumb, index) => <span key={`${crumb}-${index}`} className="flex items-center gap-2"><span className={index === classBreadcrumb.length - 1 ? "text-[#5540e8]" : ""}>{crumb}</span>{index < classBreadcrumb.length - 1 && <ChevronRight className="h-3.5 w-3.5 text-[#a7b8b2]" />}</span>)}</nav><p className="text-[10px] font-bold uppercase tracking-[.26em] text-[#53706b]">{config.eyebrow}</p><h1 className="mt-3 text-3xl font-semibold tracking-[-.05em] text-[#12304a] sm:text-5xl">{selectedClassNode ? `${selectedClassNode.name} · ${config.label}` : config.label}</h1><p className="mt-3 max-w-xl text-sm leading-6 text-[#52706b]">{config.description}{selectedClassNode ? ` ${selectedClassNode.name} sınıfına ait içerikleri ve dersleri keşfedin.` : ""}</p>{selectedClassNode && !isNews && <div className="mt-6 flex flex-wrap gap-2" aria-label="İçerik türü hızlı filtreleri">{classPageLinks.map(contentType => <button type="button" key={contentType} onClick={() => setLocation(`/icerik/${contentType}?class=${encodeURIComponent(selectedClassNode.name)}${selectedSubjectNode ? `&subject=${selectedSubjectNode.id}` : ""}`)} className="rounded-full border border-white/80 bg-white/80 px-4 py-2 text-xs font-bold text-[#526a69] shadow-sm transition hover:-translate-y-0.5 hover:border-[#bdb3f4] hover:text-[#5540e8]">{contentTypes[contentType].label}</button>)}</div>}</div><div className="rounded-3xl bg-white/80 p-4 shadow-[0_18px_45px_rgba(18,48,74,.08)]"><div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl" style={{ backgroundColor: config.accent }}><Icon className="h-5 w-5 text-[#12304a]" /></div><div><p className="text-[10px] font-bold uppercase tracking-[.2em] text-[#78918d]">{isNews ? "Blog arşivi" : "Admin kategorileri"}</p><p className="mt-1 max-w-xs text-sm font-semibold text-[#12304a]">{isNews ? "Güncel haber yazıları" : "Yalnızca aktif kategoriler"}</p></div></div></div></div></div></section>
