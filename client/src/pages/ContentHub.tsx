...
export default function ContentHub() {
  ...
  const content = ((overview.data?.content ?? []) as ContentItem[]).filter(item => (item.status === "published" || item.status === "pending") && item.contentType === type && (isNews || item.categoryId));
  ...
  const requestedPage = Number(new URLSearchParams(location.split("?")[1] ?? "").get("page") ?? 1);
  ...
  return <main className="min-h-screen bg-[#f7f6f1] text-[#12304a]">
    ...
    <section className="border-b border-[#d7e3df] bg-[#dceee9]"><div className="container py-8 sm:py-12"><div className="flex flex-wrap items-start justify-between gap-6"><div className="max-w-2xl"><p className="text-[10px] font-bold uppercase tracking-[.26em] text-[#53706b]">{config.eyebrow}</p><h1 className="mt-3 text-3xl font-semibold tracking-[-.05em] text-[#12304a] sm:text-5xl">{config.label}</h1><p className="mt-3 max-w-xl text-sm leading-6 text-[#52706b]">{config.description}</p></div><div className="rounded-3xl bg-white/80 p-4 shadow-[0_18px_45px_rgba(18,48,74,.08)]"><div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl" style={{ backgroundColor: config.accent }}><Icon className="h-5 w-5 text-[#12304a]" /></div><div><p className="text-[10px] font-bold uppercase tracking-[.2em] text-[#78918d]">{isNews ? "Blog arşivi" : "Admin kategorileri"}</p><p className="mt-1 max-w-xs text-sm font-semibold text-[#12304a]">{isNews ? "Güncel haber yazıları" : "Yalnızca aktif kategoriler"}</p></div></div></div></div></div></section>
