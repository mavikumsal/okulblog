        ...
        <section id="baslangic" className="relative overflow-hidden bg-[#06172c] pt-[86px] text-white">
          ...
          <div className="container relative grid gap-8 py-12 sm:gap-10 sm:py-16 lg:min-h-[500px] lg:grid-cols-[.95fr_1.05fr] lg:items-center lg:py-20">
            <div className="max-w-2xl">
              ...
              <h1 className="max-w-xl text-[2.8rem] font-semibold leading-[1.02] tracking-[-.055em] sm:text-5xl lg:text-[3.6rem]">Öğrenmenin en düzenli ve kolay yolu.</h1>
              <p className="mt-4 max-w-lg text-sm leading-6 text-[#c4d1d3] sm:text-lg sm:leading-7">Ders notları, testler, videolar ve soru çözümleri tek yerde. Sınıfını seç, hedefini belirle, hemen çalışmaya başla.</p>
            ...
            <div className="relative mx-auto w-full max-w-[min(100%,320px)] sm:max-w-[340px] lg:ml-auto lg:w-full lg:max-w-[560px] lg:-translate-y-3">
              ...
              <div className="relative overflow-hidden rounded-[20px] border border-white/25 bg-white/95 p-4 text-[#15344e] shadow-[0_16px_40px_rgba(31,41,55,.24)] sm:rounded-[22px] sm:p-3 lg:p-2.5">
                <div className="flex min-w-0 items-center justify-between gap-2 border-b border-[#e1e2d9] pb-3"><div className="flex items-center gap-2.5"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#cce9df] text-[#176559]"><Target size={17} /></span><div><p className="text-sm font-bold sm:text-base">Bugünkü Çalışma Planın</p><p className="text-[11px] text-[#75838a]">Gerçek ilerlemene göre hazırlandı</p></div></div><span className="rounded-full bg-[#dceee7] px-2 py-1 text-[9px] font-bold text-[#397267]">KİŞİSEL</span></div>
                <div className="mb-2 flex items-center gap-2 rounded-xl border border-[#d9e2dc] bg-[#f7faf8] px-2.5 py-2"><label htmlFor="hero-class-select" className="sr-only">Hero sınıf kategorisi seç</label><select id="hero-class-select" value={effectiveHeroClassName} onChange={event => setHeroClassName(event.target.value)} className="min-w-0 flex-1 bg-transparent text-xs font-bold text-[#24435b] outline-none"><option value={effectiveHeroClassName}>{effectiveHeroClassName}</option>{heroClassOptions.filter(node => node.name !== effectiveHeroClassName).map(node => <option key={node.id} value={node.name}>{node.name}</option>)}</select><span className="rounded-full bg-[#dceee7] px-2 py-1 text-[9px] font-extrabold text-[#397267]">CANLI</span></div><div className="rounded-[18px] bg-[#172c74] p-3 text-white sm:mt-3 sm:rounded-[20px] sm:p-3.5"><div className="flex items-center gap-4"><div className="relative grid h-20 w-20 shrink-0 place-items-center rounded-full" style={{ background: `conic-gradient(#16a39b ${personalPlan?.progressPercent ?? 0}%, rgba(255,255,255,.16) 0)` }}><div className="grid h-16 w-16 place-items-center rounded-full bg-[#172c74] text-center"><strong className="text-lg">%{personalPlan?.progressPercent ?? 0}</strong><span className="block text-[8px] text-[#cfe5df]">Tamamlandı</span></div></div><div className="min-w-0"><p className="text-[10px] font-bold tracking-[.16em] text-[#a9c9c4] uppercase">Sıradaki konu</p><p className="mt-1 text-lg font-semibold tracking-[-.03em]">{personalPlan?.title ?? heroLearning.subjectName}</p><p className="mt-1 text-xs text-[#d5e4e2]">{personalPlan?.subjectName ?? heroLearning.className}</p><p className="mt-2 text-[10px] text-[#cfe5df]">İlerlemeni kaldığın yerden sürdür</p></div></div><p className="mt-3 text-[10px] font-bold tracking-[.16em] text-[#a9c9c4] uppercase">Eğitim kategorisi</p><p className="mt-1 text-sm font-semibold">{heroLearning.subjectName} · {heroLearning.className}</p><div className="mt-3 flex flex-wrap items-center gap-1.5 text-[10px] font-bold text-[#e7eff0]"><span className="rounded-md bg-white/10 px-2 py-1">Ders</span><ChevronRight size={12} className="text-[#85aa9f]" /><span className="rounded-md bg-white/10 px-2 py-1">Ünite</span><ChevronRight size={12} className="text-[#85aa9f]" /><span className="rounded-md bg-[#e8b75e] px-2 py-1 text-[#19384f]">Kazanım</span></div></div>
