...
export default function Home() {
  ...
  const heroClassOptions = (educationCategories as HomeCategoryNode[]).filter(node => node.level === "class").sort((a, b) => (a.sortOrder ?? a.id) - (b.sortOrder ?? b.id));
  ...
  return (
    <>
      ...
      <header className="absolute inset-x-0 top-0 z-50 border-b border-white/10 bg-[#06172c]/90 text-white backdrop-blur-xl">
        <div className="container flex h-[86px] items-center justify-between gap-5">
          <button onClick={() => goTo("baslangic")} className="flex items-center gap-2.5 text-left" aria-label="OkulBlog ana sayfa">
            ...
            <nav className="hidden min-w-0 items-center justify-center gap-1 text-xs font-bold text-white/80 xl:flex">
              <button onClick={() => goTo("baslangic")} className="whitespace-nowrap rounded-lg px-3 py-2 transition-all duration-200 hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0f9f9a]/70 active:scale-95">Ana Sayfa</button>{[['Dersler','/icerik/all'],['Testler','/icerik/test'],['Dokümanlar','/icerik/document'],['Videolar','/icerik/video'],['Soru-Cevap','/soru-cevap']].map(([label, path]) => <button key={path} onClick={() => setLocation(path)} className="whitespace-nowrap rounded-lg px-3 py-2 transition-all duration-200 hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0f9f9a]/70 active:scale-95">{label}</button>)}
            </nav>

          <div className="hidden items-center gap-3 md:flex"><div className="hidden h-11 w-[270px] items-center gap-2 rounded-xl border border-white/10 bg-white/[.08] px-3 text-white/70 lg:flex"><Search size={16} /><input aria-label="Ders, konu veya içerik ara" placeholder="Ders, konu veya içerik ara..." className="min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-white/45" onKeyDown={event => { if (event.key === "Enter" && event.currentTarget.value.trim()) setLocation(`/icerikler?search=${encodeURIComponent(event.currentTarget.value.trim())}`); }} /></div>
            <Button onClick={accountAction} variant="ghost" className="font-bold text-white hover:bg-white/10 hover:text-white">{isAuthenticated ? "Panelim" : "Giriş yap"}</Button>
            <Button onClick={accountAction} className="h-11 rounded-xl bg-[#0f9f9a] px-5 font-bold text-white shadow-[0_10px_20px_rgba(15,159,154,.2)] hover:bg-[#0b817e] active:scale-[.97]">
      ...
      <main>
        ...
        <section id="baslangic" className="relative overflow-hidden bg-[#06172c] pt-[86px] text-white">
          ...
          <div className="container relative grid gap-8 py-12 sm:gap-10 sm:py-16 lg:min-h-[500px] lg:grid-cols-[.95fr_1.05fr] lg:items-center lg:py-20">
            <div className="max-w-2xl">
              <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/[.04] px-3.5 py-2 text-[11px] font-bold tracking-[.08em] text-white uppercase"><Sparkles size={14} /> {currentSlide.eyebrow || "Eğitim için düzenli bir alan"}</div>
              <h1 className="max-w-2xl text-[2.5rem] font-semibold leading-[.98] tracking-[-.065em] sm:text-6xl lg:text-[4.2rem]">{currentSlide.title}</h1>
              <p className="mt-4 max-w-lg text-sm leading-6 text-[#c4d1d3] sm:text-lg sm:leading-7">{currentSlide.description || fallbackSlide.description}</p>
              <div className="mt-3 flex flex-col gap-2 sm:mt-4 sm:flex-row sm:gap-2.5">
                <Button onClick={followSlideLink} size="lg" className="h-13 rounded-xl bg-[#0f9f9a] px-7 font-bold text-white shadow-[0_12px_28px_rgba(15,159,154,.22)] hover:bg-[#0b817e]">{currentSlide.buttonLabel || "İçerikleri keşfet"} <ArrowDownRight size={18} /></Button>
            ...
            <div className="relative mx-auto w-full max-w-[min(100%,320px)] sm:max-w-[340px] lg:ml-auto lg:w-full lg:max-w-[560px] lg:-translate-y-3">
              ...
              <div className="relative overflow-hidden rounded-[20px] border border-white/25 bg-white/95 p-4 text-[#15344e] shadow-[0_16px_40px_rgba(31,41,55,.24)] sm:rounded-[22px] sm:p-3 lg:p-2.5">
                <div className="flex min-w-0 items-center justify-between gap-2 border-b border-[#e1e2d9] pb-3"><div className="flex items-center gap-2.5"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#cce9df] text-[#176559]"><Search size={17} /></span><div><p className="text-xs font-bold sm:text-sm">Bugünkü Çalışma Planın</p><p className="text-[11px] text-[#75838a]">İlerlemeni kaldığın yerden sürdür</p></div></div><span className="rounded-full bg-[#f5e4b6] px-2 py-1 text-[9px] font-bold text-[#855f20]">AKILLI SEÇİM</span></div>
                <div className="mb-2 flex items-center gap-2 rounded-xl border border-[#d9e2dc] bg-[#f7faf8] px-2.5 py-2"><label htmlFor="hero-class-select" className="sr-only">Hero sınıf kategorisi seç</label><select id="hero-class-select" value={effectiveHeroClassName} onChange={event => setHeroClassName(event.target.value)} className="min-w-0 flex-1 bg-transparent text-xs font-bold text-[#24435b] outline-none"><option value={effectiveHeroClassName}>{effectiveHeroClassName}</option>{heroClassOptions.filter(node => node.name !== effectiveHeroClassName).map(node => <option key={node.id} value={node.name}>{node.name}</option>)}</select><span className="rounded-full bg-[#dceee7] px-2 py-1 text-[9px] font-extrabold text-[#397267]">CANLI</span></div><div className="rounded-[18px] bg-[#172c74] p-3 text-white sm:mt-3 sm:rounded-[20px] sm:p-3.5"><p className="text-[10px] font-bold tracking-[.16em] text-[#a9c9c4] uppercase">Eğitim kategorisi</p><p className="mt-1.5 text-lg font-semibold tracking-[-.03em]">{heroLearning.subjectName} · {heroLearning.className}</p><div className="mt-3 flex flex-wrap items-center gap-1.5 text-[10px] font-bold text-[#e7eff0]"><span className="rounded-md bg-white/10 px-2 py-1">Ders</span><ChevronRight size={12} className="text-[#85aa9f]" /><span className="rounded-md bg-white/10 px-2 py-1">Ünite</span><ChevronRight size={12} className="text-[#85aa9f]" /><span className="rounded-md bg-[#e8b75e] px-2 py-1 text-[#19384f]">Kazanım</span></div></div>
                ...
                <div className="mt-2 flex items-center justify-between rounded-2xl border border-[#e3e5dc] bg-white px-3 py-2.5"><div><p className="text-xs font-bold">Bugünkü çalışma planı</p><p className="mt-0.5 truncate text-[11px] text-[#77858a]">{personalPlan ? personalPlan.title : isAuthenticated ? "Henüz bir ilerleme kaydı yok" : "Üye olarak kişisel planını oluştur"}</p>{personalPlan && <p className="mt-1 text-[10px] font-bold text-[#397267]">%{personalPlan.progressPercent} tamamlandı</p>}</div>{personalPlan ? <ArrowRight size={17} className="text-[#739b90]" /> : <CheckCircle2 size={17} className="text-[#739b90]" />}</div>
              </div>
        ...
        <section aria-label="Platform istatistikleri" className="relative z-10 -mt-8 bg-transparent"><div className="container grid gap-3 px-4 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-[22px] bg-white px-5 py-5 text-center shadow-[0_14px_28px_rgba(31,41,55,.12)] ring-1 ring-[#eef0f5]"><strong className="block text-3xl font-black tracking-[-.04em] text-[#2864d8]">{homepageStats?.approvedQuestions ?? overview.data?.approvedQuestions?.length ?? 0}</strong><span className="mt-1 block text-xs font-bold text-[#6b7280]">Onaylı Soru</span></div><div className="rounded-[22px] bg-white px-5 py-5 text-center shadow-[0_14px_28px_rgba(31,41,55,.12)] ring-1 ring-[#eef0f5]"><strong className="block text-3xl font-black tracking-[-.04em] text-[#5540e8]">{homepageStats?.contentByType.find(item => item.contentType === "document")?.count ?? publishedContent.filter(item => item.contentType === "document").length}</strong><span className="mt-1 block text-xs font-bold text-[#6b7280]">Doküman</span></div><div className="rounded-[22px] bg-white px-5 py-5 text-center shadow-[0_14px_28px_rgba(31,41,55,.12)] ring-1 ring-[#eef0f5]"><strong className="block text-3xl font-black tracking-[-.04em] text-[#8b35d8]">{homepageStats?.contentByType.find(item => item.contentType === "video")?.count ?? publishedContent.filter(item => item.contentType === "video").length}</strong><span className="mt-1 block text-xs font-bold text-[#6b7280]">Video Ders</span></div><div className="rounded-[22px] bg-white px-5 py-5 text-center shadow-[0_14px_28px_rgba(31,41,55,.12)] ring-1 ring-[#eef0f5]"><strong className="block text-3xl font-black tracking-[-.04em] text-[#d62676]">{homepageStats?.activeEducationCategories ?? educationCategories.filter(category => category.isActive !== false).length}</strong><span className="mt-1 block text-xs font-bold text-[#6b7280]">Eğitim Kategorisi</span></div></div></section>
...
function ClassCoverIllustration({ group }: { group: ReturnType<typeof classGroupForName> }) {
  ...
  return <div className={`relative flex h-24 items-center justify-center overflow-hidden rounded-[20px] bg-gradient-to-br ${palette} text-white`} aria-hidden="true"><img src={classCoverUrl(group.key)} alt="" className="absolute inset-0 h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-black/35 via-black/5 to-transparent" /><span className="absolute bottom-2 left-3 text-[9px] font-extrabold tracking-[.12em]">{group.key === "elementary" ? "İLKOKUL 1–4" : group.key === "middle" ? "ORTAOKUL 5–8" : group.key === "high" ? "LİSE 9–12" : "EĞİTİM"}</span></div>;
}

export function classCoverUrl(groupKey: string) {
  ...
  return covers[groupKey] ?? covers.elementary;
}
