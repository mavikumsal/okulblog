  ...
  return (
    <>
      ...
      <main>
        ...
        <AdPlaceholder slot="home-top" className="container max-w-5xl" />
        ...
        <section id="egitim-seviyeleri" className="bg-[#f6f8fb] py-10 sm:py-14"><div className="container"><div className="flex flex-col gap-5"><div><p className="text-xs font-extrabold uppercase tracking-[.18em] text-[#0f8f8b]">Sınıfına uygun içerikleri keşfet</p><h2 className="mt-2 text-2xl font-black tracking-[-.045em] text-[#10233d] sm:text-3xl">Sınıfına uygun içerikleri keşfet</h2></div><div className="flex w-fit items-center gap-1.5 rounded-2xl border border-[#dce7e4] bg-white p-1.5 shadow-[0_8px_20px_rgba(16,46,73,.06)]" role="tablist" aria-label="Eğitim seviyesi"><button type="button" role="tab" aria-selected={selectedSchoolLevel === "elementary"} onClick={() => setSelectedSchoolLevel("elementary")} className={`rounded-xl px-5 py-2.5 text-sm font-extrabold transition ${selectedSchoolLevel === "elementary" ? "bg-[#0f9f9a] text-white shadow-[0_6px_14px_rgba(15,159,154,.2)]" : "text-[#526873] hover:bg-[#eef7f4]"}`}>İlkokul</button><button type="button" role="tab" aria-selected={selectedSchoolLevel === "middle"} onClick={() => setSelectedSchoolLevel("middle")} className={`rounded-xl px-5 py-2.5 text-sm font-extrabold transition ${selectedSchoolLevel === "middle" ? "bg-[#3d88d8] text-white shadow-[0_6px_14px_rgba(61,136,216,.2)]" : "text-[#526873] hover:bg-[#eef7f4]"}`}>Ortaokul</button><button type="button" role="tab" aria-selected={selectedSchoolLevel === "high"} onClick={() => setSelectedSchoolLevel("high")} className={`rounded-xl px-5 py-2.5 text-sm font-extrabold transition ${selectedSchoolLevel === "high" ? "bg-[#8e5bc7] text-white shadow-[0_6px_14px_rgba(142,91,199,.2)]" : "text-[#526873] hover:bg-[#eef7f4]"}`}>Lise</button></div></div><div className="mx-auto mt-8 grid max-w-[1400px] items-stretch gap-5 lg:grid-cols-[minmax(0,1fr)_320px]"><div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{visibleClassNodes.map((classNode) => { const grade = classNode.name; const group = classGroupForName(grade); const summary = classSummary(grade, educationCategories as HomeCategoryNode[], (overview.data?.content ?? []) as HomeContentItem[]); const preview = summary.previews; const counts = Object.entries(summary.counts).filter(([, count]) => count > 0).slice(0, 3); return <div key={grade} className="group relative"><div role="link" tabIndex={0} onClick={() => setLocation(buildClassAllContentUrl(grade))} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setLocation(buildClassAllContentUrl(grade)); } }} aria-label={`${grade} içeriklerini gör`} className={`flex h-full min-h-[280px] w-full cursor-pointer flex-col rounded-[22px] border p-3 text-left shadow-[0_8px_20px_rgba(31,41,55,.04)] xl:min-h-[336px] transition duration-200 hover:-translate-y-1 hover:shadow-[0_16px_30px_rgba(85,64,232,.14)] focus:outline-none focus:ring-2 focus:ring-[#5540e8] focus:ring-offset-2 ${group.classes}`}><ClassCoverIllustration group={group} grade={grade} /><div className="flex flex-1 flex-col px-2 pb-1 pt-3"><strong className="block text-sm font-extrabold text-[#17354d]">{grade}</strong><span className="mt-1 block text-[10px] font-bold uppercase tracking-[.1em] opacity-70">{summary.total} toplam içerik</span><div className="mt-auto flex flex-wrap gap-1.5 pt-5">{counts.length ? counts.map(([type, count]) => { const TypeIcon = contentTypeIcon(type); return <span key={type} className="inline-flex items-center gap-1 rounded-full bg-white/70 px-2 py-1 text-[10px] font-bold"><TypeIcon size={12} aria-hidden="true" />{contentTypeLabel(type)} {count}</span>; }) : <span className="rounded-full bg-white/70 px-2 py-1 text-[10px] font-bold">İçerik keşfet</span>}</div></div></div><div className="pointer-events-none absolute left-2 right-2 top-full z-20 mt-2 translate-y-1 rounded-2xl border border-[#e4e8e3] bg-white p-3 opacity-0 shadow-[0_16px_32px_rgba(31,41,55,.14)] transition duration-200 group-hover:pointer-events-auto group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:pointer-events-auto group-focus-within:translate-y-0 group-focus-within:opacity-100"><p className="text-[10px] font-extrabold uppercase tracking-[.12em] text-[#7b8a8d]">Yeni içerikler</p>{preview.length ? preview.map(item => <p key={item.id} className="mt-2 truncate text-xs font-bold text-[#29445a]">{item.title}</p>) : <p className="mt-2 text-xs text-[#7b8a8d]">Bu sınıfta henüz yayınlanmış içerik yok.</p>}<a href={buildClassAllContentUrl(grade)} onClick={event => event.stopPropagation()} className="mt-3 inline-flex items-center gap-1 rounded-full bg-[#5540e8] px-3 py-2 text-[11px] font-extrabold text-white transition hover:bg-[#4632cf] focus:outline-none focus:ring-2 focus:ring-[#5540e8]/40">Tümünü Gör <ArrowRight size={13} aria-hidden="true" /></a></div></div>; })}<div className="mt-6 flex justify-center"><button type="button" onClick={() => goTo("icerikler")} className="inline-flex items-center gap-2 rounded-xl border border-[#b9d8d1] bg-white px-6 py-3 text-sm font-extrabold text-[#167e79] shadow-[0_8px_18px_rgba(16,46,73,.06)] transition hover:-translate-y-0.5 hover:border-[#0f9f9a] hover:bg-[#effaf7] focus:outline-none focus:ring-2 focus:ring-[#0f9f9a]/30">Daha Fazla Göster <ArrowDown size={16} /></button></div></div><aside className="rounded-2xl border border-[#e2e8e7] bg-white p-5 shadow-[0_12px_34px_rgba(6,27,46,.08)] lg:sticky lg:top-6 lg:h-fit"><h3 className="text-lg font-black text-[#17354d]">Popüler Konular</h3>{popularTopics.slice(0, 3).map((topic, index) => <button key={topic.id} type="button" onClick={() => setLocation(`/icerikler?category=${topic.categoryId}`)} className="flex w-full items-center gap-3 border-t border-[#e2e8e7] py-4 text-left first:mt-3"><span className={`grid h-9 w-9 shrink-0 place-items-center rounded-full text-xs font-black text-white ${index === 0 ? "bg-[#0f9f9a]" : index === 1 ? "bg-[#f4b63e]" : "bg-[#7664d9]"}`}>{String(index + 1).padStart(2, "0")}</span><span className="min-w-0 flex-1"><strong className="block truncate text-sm text-[#17354d]">{topic.name}</strong><small className="mt-1 block truncate text-[11px] text-[#64748b]">{topic.contentCount} içerik · gerçek etkileşim verisi</small></span><ChevronRight size={16} className="text-[#64748b]" /></button>)}<button type="button" onClick={() => goTo("icerikler")} className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl border border-[#e2e8e7] px-3 py-2.5 text-xs font-bold text-[#17354d] transition hover:border-[#0f9f9a] hover:text-[#0f8f8b]">Tüm konuları gör <ArrowRight size={14} /></button></aside></div></div></section>

        <section aria-label="Seçilmiş içerikler" className="bg-[#f7f9fa] py-12 sm:py-16"><div className="container"><div className="mb-6 flex items-end justify-between gap-4"><h2 className="text-2xl font-black tracking-[-.045em] text-[#0b2942] sm:text-3xl">Senin için seçtiklerimiz</h2><button onClick={() => goTo("icerikler")} className="inline-flex shrink-0 items-center gap-2 text-sm font-extrabold text-[#078b87] transition hover:text-[#061b2e]">Tüm içerikleri gör <ArrowRight size={15} /></button></div>{featuredContent.length ? <div className="grid items-stretch gap-5 md:grid-cols-3">{featuredContent.slice(0, 3).map(item => <button key={item.id} onClick={() => setLocation(`/icerik/${item.contentType}/${item.id}`)} className="group flex h-full flex-col overflow-hidden rounded-2xl border border-[#e2e8e7] bg-white text-left shadow-[0_12px_34px_rgba(6,27,46,.08)] transition duration-200 hover:-translate-y-1 hover:shadow-[0_18px_38px_rgba(6,27,46,.14)] focus:outline-none focus:ring-2 focus:ring-[#08a7a2]/40"><div className={`relative grid h-[190px] place-items-center overflow-hidden ${featuredContent.indexOf(item) === 1 ? "bg-gradient-to-br from-[#e5dcff] to-[#d7eff8]" : featuredContent.indexOf(item) === 2 ? "bg-[radial-gradient(circle_at_70%,#ed9e35,#111f45_38%,#07152e)]" : "bg-gradient-to-br from-[#fff0c9] to-[#dff1df]"}`}><div className="flex items-center gap-7 text-[#17354d]" aria-hidden="true">{featuredContent.indexOf(item) === 2 ? <><span className="text-5xl">🪐</span><PlayCircle size={68} strokeWidth={1.4} className="text-white drop-shadow-[0_4px_8px_rgba(0,0,0,.25)]" /></> : featuredContent.indexOf(item) === 1 ? <><Target size={56} strokeWidth={1.4} className="text-[#8d6be8]" /><span className="text-5xl">🌍</span></> : <><FileText size={56} strokeWidth={1.4} className="text-[#b9a8d5]" /><span className="text-5xl">✏️</span></>}</div><div className="absolute inset-x-0 bottom-0 h-10 bg-gradient-to-t from-black/10 to-transparent" /></div><div className="flex flex-1 flex-col p-[18px]"><span className="inline-flex w-fit rounded-md bg-[#e7f6f4] px-2 py-1 text-[11px] font-extrabold text-[#078b87]">{contentTypeLabel(item.contentType)}</span><h3 className="mt-3 line-clamp-2 text-[17px] font-extrabold leading-6 text-[#0b2942]">{item.title}</h3><small className="mt-1 block truncate text-[13px] text-[#64748b]">{categoryNameById.get(item.categoryId ?? -1) ?? "Eğitim içeriği"} · {contentTypeLabel(item.contentType)}</small></div></button>)}</div> : <div className="mt-7 rounded-2xl border border-dashed border-[#c9d6cf] bg-white p-6 text-center text-sm text-[#71828a]">Henüz yayınlanmış seçilmiş içerik bulunmuyor.</div>}</div></section>
        ...
        <section id="duyurular" className="bg-white py-12 sm:py-14"><div className="container"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-extrabold uppercase tracking-[.18em] text-[#5540e8]">Güncel duyurular</p><h2 className="mt-3 text-3xl font-black tracking-[-.045em] text-[#111827] sm:text-4xl">Eğitim dünyasından en son haberler.</h2><p className="mt-3 text-sm text-[#6b7280]">OkulBlog’daki güncel içerikleri ve önemli gelişmeleri takip edin.</p></div><button onClick={() => setLocation("/icerik/news")} className="inline-flex items-center gap-2 self-start rounded-xl border border-[#e5e7eb] bg-white px-4 py-3 text-sm font-bold text-[#374151] shadow-sm transition hover:border-[#5540e8] hover:text-[#5540e8]">Tüm haberleri gör <ArrowRight size={16} /></button></div>{newsItems.length ? <div className="mt-10 grid gap-5 md:grid-cols-3">{newsItems.map(item => <button key={item.id} onClick={() => setLocation("/icerik/news")} className="group overflow-hidden rounded-[24px] border border-[#e5e7eb] bg-white text-left shadow-[0_8px_20px_rgba(31,41,55,.04)] transition hover:-translate-y-1 hover:shadow-[0_18px_34px_rgba(85,64,232,.12)]"><div className="h-36 bg-[#eef5ff]">{item.coverImageUrl ? <img src={item.coverImageUrl} alt={item.title} loading="lazy" className="h-full w-full object-cover transition duration-300 group-hover:scale-105" /> : <div className="grid h-full place-items-center text-[#8bb7ed]"><Megaphone size={30} /></div>}</div><div className="p-5"><div className="flex items-center justify-between gap-3"><span className="rounded-md bg-[#f0f5ff] px-2 py-1 text-[10px] font-bold text-[#2864d8]">GÜNCEL</span>{formatPublishedDate(item.createdAt) && <time dateTime={new Date(item.createdAt).toISOString()} className="text-[11px] font-semibold text-[#7b8794]">{formatPublishedDate(item.createdAt)}</time>}</div><h3 className="mt-4 line-clamp-2 text-lg font-extrabold text-[#111827]">{item.title}</h3><p className="mt-2 line-clamp-2 text-sm leading-5 text-[#6b7280]">{item.summary || "OkulBlog’dan güncel eğitim duyurusu."}</p><span className="mt-4 inline-flex items-center gap-1 text-sm font-bold text-[#2864d8]">Oku <ArrowRight size={14} /></span></div></button>)}</div> : <div className="mt-10 rounded-[24px] border border-dashed border-[#d7e1ea] bg-white px-6 py-10 text-center"><Megaphone className="mx-auto text-[#8bb7ed]" size={30} /><h3 className="mt-4 text-lg font-extrabold text-[#111827]">Henüz yayınlanmış haber yok</h3><p className="mt-2 text-sm text-[#6b7280]">Admin panelinden haber yayınlandığında kapak resmi ve içerik bilgileri burada otomatik görünecek.</p></div>}</div></section>
        ...
        {showHowItWorks && <div className="fixed inset-0 z-[80] grid place-items-center bg-[#111827]/65 p-4" role="dialog" aria-modal="true" aria-labelledby="how-it-works-title" onClick={() => setShowHowItWorks(false)}><div className="relative w-full max-w-lg rounded-[26px] bg-white p-6 text-[#17354d] shadow-2xl" onClick={event => event.stopPropagation()}><button type="button" onClick={() => setShowHowItWorks(false)} aria-label="Nasıl çalışır penceresini kapat" className="absolute right-4 top-4 grid h-9 w-9 place-items-center rounded-full text-[#71828a] transition hover:bg-[#f0f3f5] hover:text-[#17354d] focus:outline-none focus:ring-2 focus:ring-[#5540e8]"><X size={18} /></button><p className="text-[11px] font-extrabold uppercase tracking-[.16em] text-[#5540e8]">OkulBlog öğrenme akışı</p><h2 id="how-it-works-title" className="mt-2 pr-10 text-2xl font-black tracking-[-.04em]">Nasıl çalışır?</h2><div className="mt-5 grid gap-3 sm:grid-cols-3"><div className="rounded-2xl bg-[#eef6f1] p-4"><span className="text-lg font-black text-[#397267]">1</span><p className="mt-2 text-sm font-bold">Sınıfını seç</p><p className="mt-1 text-xs leading-5 text-[#6b7d82]">MEB sırasındaki sınıf ve ders yolunu aç.</p></div><div className="rounded-2xl bg-[#f5f0ff] p-4"><span className="text-lg font-black text-[#604985]">2</span><p className="mt-2 text-sm font-bold">İçerik türünü seç</p><p className="mt-1 text-xs leading-5 text-[#6b7d82]">Test, video veya dokümanla çalış.</p></div><div className="rounded-2xl bg-[#fff7df] p-4"><span className="text-lg font-black text-[#8f6621]">3</span><p className="mt-2 text-sm font-bold">İlerlemeni takip et</p><p className="mt-1 text-xs leading-5 text-[#6b7d82]">Öğrendim/Tamamladım durumunu kaydet.</p></div></div><button type="button" onClick={() => { setShowHowItWorks(false); goTo("icerikler"); }} className="mt-5 inline-flex items-center gap-2 rounded-full bg-[#5540e8] px-5 py-3 text-sm font-extrabold text-white transition hover:bg-[#4632cf] focus:outline-none focus:ring-2 focus:ring-[#5540e8]/40">İçerikleri keşfet <ArrowRight size={16} /></button></div></div>}
            ...
            <>
              <section aria-label="Bülten aboneliği" className="bg-[#f7f9fa] px-4 py-10 sm:py-14"><div className="container"><div className="rounded-[22px] border border-[#b8e3df] bg-[#e9f8f7] px-6 py-7 shadow-[0_10px_30px_rgba(15,159,154,.06)] sm:px-10 sm:py-8"><div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between md:gap-10"><div className="flex items-center gap-6"><img src="/manus-storage/okulblog-newsletter-envelope_bbef391f.png" alt="Bülten zarfı" className={`h-16 w-16 shrink-0 object-contain transition-transform duration-300 ${newsletterState === "success" ? "scale-110 rotate-[-6deg]" : ""}`} /><div><h2 className="text-xl font-black tracking-[-.03em] text-[#102c43] sm:text-2xl">Yeni içeriklerden haberdar ol.</h2><p className="mt-2 text-sm text-[#203f55]">En yeni içerikler, ipuçları ve gelişmeler e-posta adresine gelsin.</p></div></div>{newsletterState === "success" ? <div role="status" aria-live="polite" className="flex w-full max-w-[480px] items-center gap-3 rounded-xl border border-[#8bd4c7] bg-white/80 px-4 py-3 text-sm font-bold text-[#126f69] animate-in fade-in zoom-in-95 duration-300"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[#d8f5ee] text-[#0f9f9a]"><CheckCircle2 size={18} /></span><span>Aboneliğin başarıyla alındı. Yeni içerikleri kaçırmayacaksın.</span></div> : <form className="w-full max-w-[480px]" onSubmit={event => { event.preventDefault(); if (!isValidNewsletterEmail(newsletterEmail)) { setNewsletterState("error"); return; } setNewsletterState("success"); }} noValidate={false}><div className="flex"><label htmlFor="newsletter-email" className="sr-only">E-posta adresin</label><input id="newsletter-email" name="email" type="email" required value={newsletterEmail} onChange={event => { setNewsletterEmail(event.target.value); if (newsletterState === "error") setNewsletterState("idle"); }} placeholder="E-posta adresin" aria-invalid={newsletterState === "error"} className={`min-w-0 flex-1 rounded-l-xl border bg-white px-4 py-3 text-sm text-[#17354d] outline-none placeholder:text-[#87959b] focus:ring-2 focus:ring-[#0f9f9a]/20 ${newsletterState === "error" ? "border-[#d95d5d] focus:border-[#d95d5d]" : "border-[#dce5e5] focus:border-[#0f9f9a]"}`} /><button type="submit" className="rounded-r-xl bg-[#0fa49f] px-5 text-sm font-extrabold text-white transition hover:bg-[#078b87] focus:outline-none focus:ring-2 focus:ring-[#0f9f9a]/30 active:scale-[.98]">Abone Ol</button></div>{newsletterState === "error" && <p className="mt-2 text-xs font-semibold text-[#b53e3e]" role="alert">Lütfen geçerli bir e-posta adresi gir.</p>}</form>}</div></div></div></section>
              <footer className="bg-[#061b2e] text-white"><div className="container grid gap-10 py-12 sm:grid-cols-2 lg:grid-cols-[1.25fr_.8fr_.8fr_.8fr]"><div><div className="flex items-center gap-2"><span className="grid h-9 w-9 place-items-center rounded-xl bg-[#0f9f9a] text-white"><GraduationCap size={18} /></span><span className="text-base font-black text-[#8edbd4]">okul<span className="text-[#ffd21a]">blog</span></span></div><p className="mt-5 max-w-xs text-sm leading-6 text-[#a8bac5]">Öğrencilerin sınav yolculuğunda güvenilir içerik ve düzenli öğrenme alanı.</p></div><div><h3 className="text-sm font-extrabold">Keşfet</h3><div className="mt-4 grid gap-3 text-sm text-[#a8bac5]"><button onClick={() => goTo("icerikler")} className="text-left transition hover:text-white">Tüm dersler</button><button onClick={() => goTo("egitim-seviyeleri")} className="text-left transition hover:text-white">Sınıfını seç</button><button onClick={() => goTo("sinavlar")} className="text-left transition hover:text-white">Testler</button><button onClick={() => goTo("icerikler")} className="text-left transition hover:text-white">Videolar</button></div></div><div><h3 className="text-sm font-extrabold">Destek</h3><div className="mt-4 grid gap-3 text-sm text-[#a8bac5]"><button onClick={() => setLocation("/soru-cevap")} className="text-left transition hover:text-white">Soru-Cevap</button><button onClick={() => setLocation("/destek/sss")} className="text-left transition hover:text-white">Sık Sorulan Sorular</button><button onClick={() => setLocation("/soru-cevap#iletisim")} className="text-left transition hover:text-white">İletişim</button></div></div><div><h3 className="text-sm font-extrabold">Kurumsal</h3><div className="mt-4 grid gap-3 text-sm text-[#a8bac5]"><button onClick={() => setLocation("/hakkimizda")} className="text-left transition hover:text-white">Hakkımızda</button><button onClick={() => setLocation("/gizlilik")} className="text-left transition hover:text-white">Gizlilik</button><button onClick={() => setLocation("/kullanim-kosullari")} className="text-left transition hover:text-white">Kullanım Koşulları</button></div></div></div><div className="container flex flex-col gap-4 border-t border-white/10 py-5 text-xs text-[#8295a2] sm:flex-row sm:items-center sm:justify-between"><span>© {new Date().getFullYear()} OkulBlog. Tüm hakları saklıdır.</span><div className="flex items-center gap-2" aria-label="Sosyal medya hesapları"><a href="https://www.instagram.com/" target="_blank" rel="noreferrer" aria-label="Instagram" className="grid h-9 w-9 place-items-center rounded-full border border-white/10 text-[#a8bac5] transition-all duration-200 hover:-translate-y-0.5 hover:border-[#f2c866] hover:bg-[#f2c866] hover:text-[#061b2e] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#f2c866]"><Instagram size={16} /></a><a href="https://www.facebook.com/" target="_blank" rel="noreferrer" aria-label="Facebook" className="grid h-9 w-9 place-items-center rounded-full border border-white/10 text-[#a8bac5] transition-all duration-200 hover:-translate-y-0.5 hover:border-[#b8ddd4] hover:bg-[#b8ddd4] hover:text-[#061b2e] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#b8ddd4]"><Facebook size={16} /></a><a href="https://www.youtube.com/" target="_blank" rel="noreferrer" aria-label="YouTube" className="grid h-9 w-9 place-items-center rounded-full border border-white/10 text-[#a8bac5] transition-all duration-200 hover:-translate-y-0.5 hover:border-[#f3c7bd] hover:bg-[#f3c7bd] hover:text-[#061b2e] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#f3c7bd]"><Youtube size={16} /></a><a href="https://www.linkedin.com/" target="_blank" rel="noreferrer" aria-label="LinkedIn" className="grid h-9 w-9 place-items-center rounded-full border border-white/10 text-[#a8bac5] transition-all duration-200 hover:-translate-y-0.5 hover:border-[#d8c8e8] hover:bg-[#d8c8e8] hover:text-[#061b2e] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#d8c8e8]"><Linkedin size={16} /></a></div></div></footer>
            </>
      </div>
    </>
  );
...
const NEW_CONTENT_VISUALS = [
  { src: "/manus-storage/okulblog-new-document_b4ab2af4.jpg", alt: "Çalışma notu ve kalem illüstrasyonu" },
  { src: "/manus-storage/okulblog-new-math_776a1f2f.jpg", alt: "Matematik geometri illüstrasyonu" },
  { src: "/manus-storage/okulblog-new-science_17698c58.jpg", alt: "Fen bilimleri laboratuvar illüstrasyonu" },
  { src: "/manus-storage/okulblog-new-social_88e5c484.jpg", alt: "Sosyal bilgiler ve tarih illüstrasyonu" },
] as const;

export function isNewContent(value?: string | number | Date, now: string | number | Date = Date.now()) {
  if (value == null) return false;
  const publishedAt = new Date(value).getTime();
  const currentTime = new Date(now).getTime();
  if (!Number.isFinite(publishedAt) || !Number.isFinite(currentTime) || publishedAt > currentTime) return false;
  const age = currentTime - publishedAt;
  return age <= 7 * 24 * 60 * 60 * 1000;
}

function formatPublishedDate(value?: string | number | Date) {
