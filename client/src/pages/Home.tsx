  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-50 border-b border-[#eef0f5] bg-white/95 backdrop-blur-xl">
        <div className="container flex h-[76px] items-center justify-between gap-5">
          <button onClick={() => goTo("baslangic")} className="flex items-center gap-2.5 text-left" aria-label="OkulBlog ana sayfa">
            ...
            <nav className="hidden items-center gap-3 text-[11px] font-bold text-[#374151] xl:flex">
              <button onClick={() => goTo("icerikler")} className="transition-colors hover:text-[#5540e8]">Ana Sayfa</button>
              <button onClick={() => setLocation("/panel/testler")} className="transition-colors hover:text-[#5540e8]">Testler</button>
              <button onClick={() => setLocation("/panel/dokumanlar")} className="transition-colors hover:text-[#5540e8]">Dokümanlar</button>
              <button onClick={() => setLocation("/panel/videolar")} className="transition-colors hover:text-[#5540e8]">Videolar</button>
              <button onClick={() => setLocation("/panel/oyunlar")} className="transition-colors hover:text-[#5540e8]">Oyunlar</button>
              <button onClick={() => setLocation("/panel/simulasyonlar")} className="transition-colors hover:text-[#5540e8]">Simülasyonlar</button>
              <button onClick={() => setLocation("/panel/haberler")} className="transition-colors hover:text-[#5540e8]">Haberler</button>
              <button onClick={() => setLocation("/soru-cevap")} className="transition-colors hover:text-[#5540e8]">Soru-Cevap</button>
            </nav>
        ...
        {menuOpen && <div className="border-t border-[#eef0f5] bg-white px-4 py-4 md:hidden"><div className="grid gap-1"><button onClick={() => goTo("icerikler")} className="rounded-xl px-4 py-3 text-left font-bold hover:bg-[#f4f2ff]">Ana Sayfa</button>{[["Testler", "/panel/testler"], ["Dokümanlar", "/panel/dokumanlar"], ["Videolar", "/panel/videolar"], ["Oyunlar", "/panel/oyunlar"], ["Simülasyonlar", "/panel/simulasyonlar"], ["Haberler", "/panel/haberler"], ["Soru-Cevap", "/soru-cevap"]].map(([label, path]) => <button key={path} onClick={() => setLocation(path)} className="rounded-xl px-4 py-3 text-left font-bold hover:bg-[#f4f2ff]">{label}</button>)}<Button onClick={accountAction} className="mt-2 bg-[#5540e8]">{getHomeAccountLabel(isAuthenticated, loading)}</Button></div></div>}
      </header>
