...
export default function Home() {
  ...
  const submitContact = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!contactForm.name.trim()) errors.name = "Adınızı yazın.";
    if (!contactForm.email.trim()) errors.email = "E-posta adresinizi yazın.";
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(contactForm.email.trim())) errors.email = "Geçerli bir e-posta adresi girin.";
    if (!contactForm.message.trim()) errors.message = "Mesajınızı yazın.";
  ...
  return (
    <>
      ...
      <main>
        ...
        <section id="soru-cevap" className="border-t border-[#dfe4dc] bg-[#f7f8f2]"><div className="container grid gap-8 py-16 lg:grid-cols-[1fr_1fr] lg:items-start"><div><p className="editorial-kicker text-[#5c877e]">Topluluk alanı</p><h2 className="mt-3 text-4xl font-semibold tracking-[-.055em] text-[#102e49]">Takıldığın yerde birlikte düşünelim.</h2><p className="mt-4 max-w-xl text-sm leading-6 text-[#71838b]">Sorularını ve kaynak görsellerini paylaş; üyeler soru ve cevaplarla birbirine destek olsun.</p><Button onClick={() => setLocation("/soru-cevap")} className="mt-6 rounded-full bg-[#18344f]">Soru-Cevap alanına git <ArrowRight size={16} /></Button><p className="mt-3 text-xs text-[#8a999b]">{qaPreview.data?.questions.length ?? 0} yayınlanmış soru</p></div><div className="rounded-[26px] border border-[#e1e5dc] bg-white p-6 shadow-[0_14px_30px_rgba(61,77,91,.06)]"><div className="flex items-center justify-between"><div><p className="text-xs font-bold tracking-[.16em] text-[#7b928f] uppercase">İletişim</p><h3 className="mt-2 text-2xl font-bold text-[#29465a]">OkulBlog ekibi burada.</h3></div><MessageCircle size={25} className="text-[#47736a]" /></div><p className="mt-3 text-sm leading-6 text-[#71838b]">{siteContact.data?.contact_description || "Öneri, iş birliği ve destek talepleriniz için bize ulaşın."}</p>{Object.keys(contactErrors).length > 0 && <div role="alert" className="mt-4 rounded-2xl border border-[#f1c6c0] bg-[#fff4f1] p-4 text-sm font-semibold text-[#9a4b3d]"><p>Lütfen formdaki alanları kontrol edin.</p><ul className="mt-2 list-disc space-y-1 pl-5 font-medium">{Object.values(contactErrors).map(error => <li key={error}>{error}</li>)}</ul></div>}{contactSubmitted ? <div className="mt-5 rounded-2xl bg-[#e7f2ec] p-4 text-sm font-semibold text-[#286d60]"><CheckCircle2 className="mr-2 inline-block h-4 w-4" />Mesajınız için teşekkürler. E-posta uygulamanız açılmadıysa bize doğrudan yazabilirsiniz.</div> : <form onSubmit={submitContact} className="mt-5 space-y-3"><div className="grid gap-3 sm:grid-cols-2"><label className="space-y-1 text-xs font-bold text-[#52666c]">Adınız<input required aria-label="Adınız" aria-invalid={Boolean(contactErrors.name)} value={contactForm.name} onChange={event => { setContactForm(form => ({ ...form, name: event.target.value })); setContactErrors(errors => { const next = { ...errors }; delete next.name; return next; }); }} className={`mt-1 h-10 w-full rounded-xl border px-3 text-sm font-normal outline-none focus:ring-2 focus:ring-[#5540e8] ${contactErrors.name ? "border-[#d9796b]" : "border-[#d8e6de]"}`} />{contactErrors.name && <span className="block text-[11px] font-semibold text-[#b45545]">{contactErrors.name}</span>}</label><label className="space-y-1 text-xs font-bold text-[#52666c]">E-posta<input required aria-label="E-posta" aria-invalid={Boolean(contactErrors.email)} type="email" value={contactForm.email} onChange={event => { setContactForm(form => ({ ...form, email: event.target.value })); setContactErrors(errors => { const next = { ...errors }; delete next.email; return next; }); }} className={`mt-1 h-10 w-full rounded-xl border px-3 text-sm font-normal outline-none focus:ring-2 focus:ring-[#5540e8] ${contactErrors.email ? "border-[#d9796b]" : "border-[#d8e6de]"}`} />{contactErrors.email && <span className="block text-[11px] font-semibold text-[#b45545]">{contactErrors.email}</span>}</label></div><label className="block space-y-1 text-xs font-bold text-[#52666c]">Mesajınız<textarea required aria-label="Mesajınız" aria-invalid={Boolean(contactErrors.message)} rows={4} value={contactForm.message} onChange={event => { setContactForm(form => ({ ...form, message: event.target.value })); setContactErrors(errors => { const next = { ...errors }; delete next.message; return next; }); }} className={`mt-1 w-full rounded-xl border px-3 py-2 text-sm font-normal outline-none focus:ring-2 focus:ring-[#5540e8] ${contactErrors.message ? "border-[#d9796b]" : "border-[#d8e6de]"}`} />{contactErrors.message && <span className="block text-[11px] font-semibold text-[#b45545]">{contactErrors.message}</span>}</label><Button type="submit" className="rounded-xl bg-[#5540e8] font-bold hover:bg-[#4632cf]"><Send size={15} /> Mesaj gönder</Button></form>}</div></div></section>
