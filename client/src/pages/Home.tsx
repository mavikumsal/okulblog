  ...
  return (
    <>
      ...
      <main>
        ...
        <section id="egitim-seviyeleri" className="border-b border-[#e0e1d9] bg-[#f7f4ed] py-16 sm:py-20"><div className="container"><div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="editorial-kicker text-[#5c877e]">Eğitim yolunu seç</p><h2 className="mt-3 text-3xl font-semibold tracking-[-.055em] text-[#102e49] sm:text-4xl">Kendi seviyende keşfet.</h2><p className="mt-3 max-w-xl text-sm leading-6 text-[#6b7d82]">İlkokuldan liseye, çalışma düzenini sana en uygun öğrenme alanıyla başlat.</p></div><span className="rounded-full border border-[#d9e2dc] bg-white px-4 py-2 text-xs font-bold text-[#568078]">1–12. sınıflar</span></div><div className="grid gap-4 lg:grid-cols-3"><LevelCard eyebrow="İLKOKUL" title="Keşfederek öğren" description="Temel beceriler, renkli etkinlikler ve eğlenceli öğrenme deneyimi." tone="peach" slice="left" grades={["1. Sınıf", "2. Sınıf", "3. Sınıf", "4. Sınıf"]} tags={["Türkçe", "Matematik", "Hayat Bilgisi"]} onClick={() => (isAuthenticated ? setLocation("/panel/kategoriler") : startLogin())} /><LevelCard eyebrow="ORTAOKUL" title="Bilgini güçlendir" description="Konu anlatımı, soru bankası ve sınav odaklı ilerleme alanı." tone="lavender" slice="center" grades={["5. Sınıf", "6. Sınıf", "7. Sınıf", "8. Sınıf"]} tags={["Fen", "Sosyal", "LGS Hazırlık"]} onClick={() => (isAuthenticated ? setLocation("/panel/kategoriler") : startLogin())} /><LevelCard eyebrow="LİSE" title="Hedefine hazırlan" description="9–12. sınıf dersleri ve üniversite sınavına hazırlık tek merkezde." tone="sky" slice="right" grades={["9. Sınıf", "10. Sınıf", "11. Sınıf", "12. Sınıf"]} tags={["TYT", "AYT", "Denemeler"]} onClick={() => (isAuthenticated ? setLocation("/panel/kategoriler") : startLogin())} /></div><div className="mt-5 grid gap-px overflow-hidden rounded-[24px] border border-[#e0e4df] bg-[#e0e4df] sm:grid-cols-4"><QuickStat icon="🎓" title="1–12" detail="Tüm sınıf seviyeleri" /><QuickStat icon="🎯" title="LGS" detail="Hedef odaklı hazırlık" /><QuickStat icon="📘" title="TYT · AYT" detail="Sınav çalışma alanı" /><QuickStat icon="📈" title="360°" detail="Gelişim ve raporlama" /></div></div></section>
        ...
        <section id="icerikler" className="container py-20 sm:py-28">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{featuredContent.length ? featuredContent.map(item => <button key={item.id} onClick={() => item.contentType === "test" ? setLocation(`/test/${item.id}`) : (isAuthenticated ? setLocation("/panel/icerikler") : startLogin())} className="group overflow-hidden rounded-[26px] border border-[#dfe1d9] bg-white text-left transition duration-200 hover:-translate-y-1 hover:shadow-[0_16px_30px_rgba(16,46,73,.08)]"><div className="h-36 overflow-hidden bg-[#e8eee8]">{item.coverImageUrl ? <img src={item.coverImageUrl} alt="" className="h-full w-full object-cover transition duration-300 group-hover:scale-105" /> : <div className="grid h-full place-items-center text-[#68877e]"><BookOpen size={30} /></div>}</div><div className="p-5"><div className="flex items-center justify-between gap-2"><span className="text-[10px] font-extrabold tracking-[.14em] text-[#6c8d84] uppercase">{contentTypeLabel(item.contentType)}</span><ArrowRight size={16} className="text-[#9aabad]" /></div><h3 className="mt-3 line-clamp-2 text-xl font-bold tracking-[-.04em] text-[#17354d]">{item.title}</h3><div className="mt-3 flex flex-wrap gap-2"><span className="rounded-full bg-[#e8f2ed] px-2.5 py-1 text-[10px] font-bold text-[#397266]">Eğitim · {categoryNameById.get(item.categoryId ?? 0) ?? "Kategori"}</span>{item.institutionCategoryId ? <span className="rounded-full bg-[#f8edcf] px-2.5 py-1 text-[10px] font-bold text-[#8b681e]">Kurum · {categoryNameById.get(item.institutionCategoryId) ?? "Kurum kategorisi"}</span> : null}</div></div></button>) : contentAreas.map(({ name, detail, path, icon: Icon, accent, ink }) => <button key={name} onClick={() => (isAuthenticated ? setLocation(path) : startLogin())} className="group min-h-[220px] bg-[#fbfaf6] p-6 text-left transition duration-200 hover:bg-white"><div className="flex items-start justify-between"><span className={`grid h-12 w-12 place-items-center rounded-2xl ${accent} ${ink}`}><Icon size={21} /></span><ArrowRight size={18} className="text-[#a3afb0] transition-transform group-hover:translate-x-1 group-hover:text-[#102e49]" /></div><h3 className="mt-14 text-2xl font-bold tracking-[-.045em] text-[#17354d]">{name}</h3><p className="mt-2 text-sm text-[#71828a]">{detail}</p></button>)}</div>
        </section>
