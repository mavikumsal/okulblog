...
import { getPanelContentType, panelContentTypeByRoute, panelContentTypeLabels } from "@shared/panelModules";
...
function PanelContent() {
  ...
  const section = requestedSection === "ayarlar" && !isAdmin ? "restricted-settings" : sectionAlias[requestedSection] ?? requestedSection;
  ...
  const updateCategory = trpc.categories.update.useMutation({
    onSuccess: () => { setEditingCategoryId(null); setEditingCategoryName(""); utils.categories.list.invalidate(); utils.platform.overview.invalidate(); toast.success("Kategori adı güncellendi."); },
    onError: () => toast.error("Kategori adı güncellenemedi."),
  });
  const updateCategoryStatus = trpc.categories.setStatus.useMutation({
    onSuccess: () => { utils.categories.list.invalidate(); utils.platform.overview.invalidate(); toast.success("Kategori durumu güncellendi."); },
  ...
  const selectedContentType = getPanelContentType(requestedSection) ?? contentType;
  const contentList = trpc.contents.list.useQuery({ contentType: selectedContentType }, { enabled: Boolean(user) && section === "icerikler" && isAllowed(panelContentTypeLabels[selectedContentType]) });
  const [contentCategoryId, setContentCategoryId] = useState("");
  const createContent = trpc.contents.create.useMutation({
    onSuccess: () => { setContentTitle(""); setContentSummary(""); utils.platform.overview.invalidate(); utils.contents.list.invalidate({ contentType: selectedContentType }); toast.success("İçerik taslak olarak kaydedildi."); },
    onError: () => toast.error("İçerik kaydedilemedi."),
  });
  const archiveContent = trpc.contents.archive.useMutation({
    onSuccess: () => { utils.contents.list.invalidate({ contentType: selectedContentType }); utils.platform.overview.invalidate(); toast.success("İçerik arşivlendi."); },
  ...
  const updateUserRole = trpc.admin.updateUserRole.useMutation({
    onSuccess: async () => { await refreshAdminUsers(utils); toast.success("Kullanıcı rolü güncellendi."); },
  ...
  const homeSlides = trpc.admin.homeSlides.useQuery(undefined, { enabled: isAdmin });
  const popularEducationCategories = trpc.admin.popularEducationCategories.useQuery(undefined, { enabled: isAdmin });
  const [popularCategoryIds, setPopularCategoryIds] = useState<number[]>([]);
  useEffect(() => {
    if (popularEducationCategories.data) setPopularCategoryIds(popularEducationCategories.data.selectedIds);
  }, [popularEducationCategories.data]);
  const savePopularEducationCategories = trpc.admin.savePopularEducationCategories.useMutation({
    onSuccess: () => { utils.admin.popularEducationCategories.invalidate(); utils.platform.popularEducationCategories.invalidate(); toast.success("Popüler eğitim kategorileri kaydedildi."); },
    onError: () => toast.error("Popüler eğitim kategorileri kaydedilemedi."),
  });
  const togglePopularCategory = (categoryId: number) => setPopularCategoryIds(current => current.includes(categoryId) ? current.filter(id => id !== categoryId) : current.length >= 12 ? current : [...current, categoryId]);
  const movePopularCategory = (index: number, direction: -1 | 1) => setPopularCategoryIds(current => { const target = index + direction; if (target < 0 || target >= current.length) return current; const next = [...current]; [next[index], next[target]] = [next[target], next[index]]; return next; });
  ...
  const resetSlideForm = () => {
    setEditingSlideId(null); setSlideEyebrow(""); setSlideTitle(""); setSlideDescription(""); setSlideButtonLabel(""); setSlideButtonLink(""); setSlideImageUrl(""); setSlideSortOrder("0"); setSlideIsActive(true);
  };
  const sliderPayload = () => ({ eyebrow: slideEyebrow.trim() || null, title: slideTitle.trim(), description: slideDescription.trim() || null, buttonLabel: slideButtonLabel.trim() || null, buttonLink: slideButtonLink.trim() || null, imageUrl: slideImageUrl.trim() || null, sortOrder: Number(slideSortOrder || 0), isActive: slideIsActive });
  const createHomeSlide = trpc.admin.createHomeSlide.useMutation({
    onSuccess: () => { resetSlideForm(); utils.admin.homeSlides.invalidate(); utils.platform.homeSlides.invalidate(); toast.success("Slider kaydı oluşturuldu."); },
    onError: () => toast.error("Slider kaydı oluşturulamadı. Görsel bağlantısını kontrol edin."),
  });
  const updateHomeSlide = trpc.admin.updateHomeSlide.useMutation({
    onSuccess: () => { resetSlideForm(); utils.admin.homeSlides.invalidate(); utils.platform.homeSlides.invalidate(); toast.success("Slider kaydı güncellendi."); },
    onError: () => toast.error("Slider kaydı güncellenemedi."),
  });
  const deleteHomeSlide = trpc.admin.deleteHomeSlide.useMutation({
    onSuccess: () => { resetSlideForm(); utils.admin.homeSlides.invalidate(); utils.platform.homeSlides.invalidate(); toast.success("Slider kaydı silindi."); },
    onError: () => toast.error("Slider kaydı silinemedi."),
  });
  const editSlide = (slide: NonNullable<typeof homeSlides.data>[number]) => {
    setEditingSlideId(slide.id); setSlideEyebrow(slide.eyebrow ?? ""); setSlideTitle(slide.title); setSlideDescription(slide.description ?? ""); setSlideButtonLabel(slide.buttonLabel ?? ""); setSlideButtonLink(slide.buttonLink ?? ""); setSlideImageUrl(slide.imageUrl ?? ""); setSlideSortOrder(String(slide.sortOrder)); setSlideIsActive(slide.isActive);
  };
  ...
  const titleMap: Record<string, { eyebrow: string; title: string; text: string }> = {
    ...
    "ana-sayfa-yonetimi": { eyebrow: "Ana sayfa yönetimi", title: "Ana sayfanızın vitrini burada.", text: "Slider, popüler eğitim kategorileri, SEO, reklam ve site haritası ayarlarını tek merkezden yönetin." },
    "kurum-kategorisi": { eyebrow: "Kurum kategorisi", title: "Kamu ve kurum sınavlarını ayrı yönetin.", text: "KPSS ve kamu kurumu sınavları için bağımsız alt kategori ağacını yönetin." },
    testler: { eyebrow: "Test yönetimi", title: "Test içeriklerini tek merkezden yönetin.", text: "Test taslaklarını kategori, soru ve yayın akışıyla düzenleyin." },
    dokumanlar: { eyebrow: "Doküman yönetimi", title: "Kaynak arşivinizi düzenleyin.", text: "Doküman taslaklarını eğitim ve kurum kategorileriyle ilişkilendirin." },
    videolar: { eyebrow: "Video yönetimi", title: "Video içeriklerini yönetin.", text: "Anlatım videolarını kategori ve yayın durumuyla takip edin." },
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8 pb-12">
      ...
      {section === "icerikler" && <section className="grid gap-5 lg:grid-cols-[.9fr_1.1fr]"><div className="rounded-[24px] border border-[#e6e6de] bg-white p-6"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-[#e0ebf5] text-[#3b6987]"><BookOpenCheck size={20} /></span><div><h2 className="font-bold text-[#29465a]">Yeni içerik</h2><p className="text-xs text-[#71838b]">İçerik türünü seçerek taslak oluşturun.</p></div></div><div className="mt-6 space-y-4"><div className="space-y-2"><Label>İçerik türü</Label><select value={selectedContentType} disabled={Boolean(panelContentTypeByRoute[requestedSection])} onChange={event => setContentType(event.target.value as typeof contentType)} className="h-11 w-full rounded-xl border border-input bg-background px-3 text-sm"><option value="test">Testler</option><option value="document">Dokümanlar</option><option value="simulation">Simülasyonlar</option><option value="video">Videolar</option><option value="game">Oyunlar</option><option value="news">Haberler</option></select></div><div className="space-y-2"><Label>Kategori</Label><select value={contentCategoryId} onChange={event => setContentCategoryId(event.target.value)} className="h-11 w-full rounded-xl border border-input bg-background px-3 text-sm"><option value="">Kategori seçin (isteğe bağlı)</option>{(categoryNodes.data ?? []).filter(item => item.categoryType === "institution" ? item.isActive : true).map(item => <option key={item.id} value={item.id}>{item.categoryType === "education" ? "Eğitim" : "Kurum"} · {item.name}</option>)}</select></div><div className="space-y-2"><Label htmlFor="contentTitle">Başlık</Label><Input id="contentTitle" value={contentTitle} onChange={event => setContentTitle(event.target.value)} placeholder="İçeriğin başlığı" className="h-11 rounded-xl" /></div><div className="space-y-2"><Label htmlFor="contentSummary">Kısa açıklama</Label><Textarea id="contentSummary" value={contentSummary} onChange={event => setContentSummary(event.target.value)} placeholder="İçeriğin ne sunduğunu açıklayın." className="min-h-25 rounded-xl" /></div><Button disabled={contentTitle.trim().length < 3 || createContent.isPending} onClick={() => createContent.mutate({ title: contentTitle, contentType: selectedContentType, summary: contentSummary, categoryId: contentCategoryId ? Number(contentCategoryId) : undefined })} className="w-full rounded-xl bg-[#18344f]">{createContent.isPending ? "Kaydediliyor..." : "İçerik taslağı oluştur"}<Plus size={16} /></Button></div></div><div className="rounded-[24px] border border-[#e6e6de] bg-white p-6"><p className="text-sm font-bold text-[#29465a]">İçerik ilkeleri</p><div className="mt-5 space-y-3">{[["Testler", "Soru havuzundaki onaylı sorularla bağlanır."], ["Dokümanlar", "S3 bulut depolama alanına yüklenen dosyalarla ilişkilendirilir."], ["Videolar", "Güvenli video bağlantısı veya dosya ile sunulur."], ["Haberler", "Haber kategorileri altında düzenlenir."]].map(([name, text]) => <div key={name} className="rounded-xl bg-[#f7f8f4] p-4"><p className="text-sm font-bold text-[#496374]">{name}</p><p className="mt-1 text-xs leading-5 text-[#77878d]">{text}</p></div>)}</div></div></section>}

      {section === "icerikler" && <section className="rounded-[24px] border border-[#e6e6de] bg-white p-6"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold tracking-[.16em] text-[#7b928f] uppercase">{panelContentTypeLabels[selectedContentType]} kayıtları</p><h2 className="mt-2 text-xl font-bold text-[#29465a]">Bu modülün içerik arşivi</h2><p className="mt-2 text-sm leading-6 text-[#71838b]">Oluşturulan taslakları bu modüle özel olarak görüntüleyin.</p></div><Badge variant="outline" className="shrink-0">{contentList.data?.length ?? 0} kayıt</Badge></div><div className="mt-6 space-y-3">{contentList.isLoading ? <div className="rounded-xl bg-[#f7f8f4] p-5 text-sm text-[#728087]">{panelContentTypeLabels[selectedContentType]} kayıtları yükleniyor...</div> : contentList.isError ? <div className="rounded-xl bg-[#fff3ef] p-5 text-sm text-[#a65345]">Kayıtlar yüklenemedi. Lütfen tekrar deneyin.</div> : (contentList.data ?? []).length ? contentList.data?.map(item => <div key={item.id} className="rounded-xl border border-[#eef0eb] px-4 py-4"><div className="flex items-start justify-between gap-4"><div><p className="text-sm font-semibold text-[#365368]">{item.title}</p><p className="mt-1 text-xs text-[#7b8b90]">{item.summary || "Açıklama eklenmedi."}</p></div><div className="flex items-center gap-2"><Badge variant="outline" className="shrink-0 text-[10px]">{item.status === "published" ? "Yayında" : item.status === "archived" ? "Arşiv" : "Taslak"}</Badge>{item.status !== "archived" && <Button variant="outline" size="sm" className="h-8 rounded-lg text-xs" disabled={archiveContent.isPending} onClick={() => archiveContent.mutate({ id: item.id, contentType: selectedContentType })}>Arşivle</Button>}</div></div></div>) : <div className="rounded-xl bg-[#f7f8f4] p-5 text-sm leading-6 text-[#728087]">Bu modülde henüz kayıt bulunmuyor. Yukarıdaki formdan ilk taslağı oluşturabilirsiniz.</div>}</div></section>}
      ...
      {section === "ayarlar" && <section className="rounded-[24px] border border-[#e6e6de] bg-white p-6"><div className="mb-8 rounded-2xl border border-[#dfe9e2] bg-[#f6faf6] p-5"><div className="flex items-start justify-between gap-4"><div><h2 className="text-lg font-bold text-[#29465a]">Popüler eğitim kategorileri</h2><p className="mt-1 text-sm leading-6 text-[#71838b]">Ana sayfada gösterilecek kategorileri seçin ve sıralayın.</p></div><Badge className="border-0 bg-[#e6f2ed] text-[#307363]">{popularCategoryIds.length} seçili</Badge></div>{isAdmin ? <div className="mt-4 grid gap-5 lg:grid-cols-[1fr_.9fr]"><div className="grid gap-2 sm:grid-cols-2">{popularEducationCategories.data?.available.map(category => { const selected = popularCategoryIds.includes(category.id); return <button key={category.id} type="button" onClick={() => togglePopularCategory(category.id)} className={`flex items-center justify-between rounded-xl border px-3 py-2.5 text-left transition ${selected ? "border-[#9bcabc] bg-white" : "border-[#e6e9e2] bg-white/60 hover:border-[#c8ddd5]"}`}><span className="truncate text-sm font-semibold text-[#365368]">{category.name}</span><span className={`ml-2 grid h-6 w-6 shrink-0 place-items-center rounded-full text-xs font-bold ${selected ? "bg-[#2f7668] text-white" : "border border-[#cbd8d1] text-transparent"}`}>✓</span></button>; })}</div><div className="rounded-xl bg-white p-3"><p className="text-xs font-bold text-[#365368]">Ana sayfa sırası</p><div className="mt-2 space-y-2">{popularCategoryIds.length ? popularCategoryIds.map((id, index) => { const category = popularEducationCategories.data?.available.find(item => item.id === id); return <div key={id} className="flex items-center gap-2 rounded-lg border border-[#e4e8e1] px-2.5 py-2"><span className="text-xs font-bold text-[#286d60]">{index + 1}</span><span className="min-w-0 flex-1 truncate text-xs font-semibold text-[#365368]">{category?.name ?? `Kategori #${id}`}</span><button type="button" onClick={() => movePopularCategory(index, -1)} disabled={index === 0} className="text-xs disabled:opacity-30" aria-label="Yukarı taşı">↑</button><button type="button" onClick={() => movePopularCategory(index, 1)} disabled={index === popularCategoryIds.length - 1} className="text-xs disabled:opacity-30" aria-label="Aşağı taşı">↓</button><button type="button" onClick={() => togglePopularCategory(id)} className="text-xs text-[#a05046]" aria-label="Listeden çıkar">×</button></div>; }) : <p className="py-3 text-xs text-[#7b8b90]">Henüz kategori seçilmedi.</p>}</div><Button onClick={() => savePopularEducationCategories.mutate({ categoryIds: popularCategoryIds })} disabled={savePopularEducationCategories.isPending} className="mt-3 w-full rounded-xl bg-[#18344f]">{savePopularEducationCategories.isPending ? "Kaydediliyor..." : "Kaydet"}</Button></div></div> : <RestrictedNotice />}</div><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><h2 className="text-xl font-bold text-[#29465a]">Ana sayfa slider</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#71838b]">Ana sayfadaki öne çıkan alanları buradan oluşturun. Sıralama değeri küçük olan kayıt önce gösterilir; pasif kayıtlar ana sayfada görünmez.</p></div><Badge className="w-fit border-0 bg-[#e6f2ed] text-[#307363] hover:bg-[#e6f2ed]">Admin yönetimi</Badge></div>{isAdmin ? <div className="mt-6 grid gap-7 lg:grid-cols-[.88fr_1.12fr]"><div className="rounded-2xl bg-[#f6f7f2] p-4 sm:p-5"><p className="text-sm font-bold text-[#365368]">{editingSlideId ? "Slider kaydını düzenle" : "Yeni slider kaydı"}</p><div className="mt-4 space-y-3"><Input value={slideEyebrow} onChange={event => setSlideEyebrow(event.target.value)} placeholder="Üst başlık · Örn. SINAV HAZIRLIĞI" className="h-11 rounded-xl bg-white" /><Input value={slideTitle} onChange={event => setSlideTitle(event.target.value)} placeholder="Başlık" className="h-11 rounded-xl bg-white" /><Textarea value={slideDescription} onChange={event => setSlideDescription(event.target.value)} placeholder="Kısa açıklama" className="min-h-20 rounded-xl bg-white" /><div className="grid gap-3 sm:grid-cols-2"><Input value={slideButtonLabel} onChange={event => setSlideButtonLabel(event.target.value)} placeholder="Buton metni" className="h-11 rounded-xl bg-white" /><Input value={slideButtonLink} onChange={event => setSlideButtonLink(event.target.value)} placeholder="Bağlantı · /panel" className="h-11 rounded-xl bg-white" /></div><Input value={slideImageUrl} onChange={event => setSlideImageUrl(event.target.value)} placeholder="Görsel URL'si · isteğe bağlı" className="h-11 rounded-xl bg-white" /><div className="flex items-center justify-between gap-4"><div className="flex items-center gap-3"><Label htmlFor="slideSortOrder" className="text-xs">Sıralama</Label><Input id="slideSortOrder" type="number" min="0" value={slideSortOrder} onChange={event => setSlideSortOrder(event.target.value)} className="h-10 w-20 rounded-xl bg-white" /></div><div className="flex items-center gap-3"><Label htmlFor="slideActive" className="text-xs">Ana sayfada göster</Label><Switch id="slideActive" checked={slideIsActive} onCheckedChange={setSlideIsActive} /></div></div><div className="flex gap-2"><Button disabled={slideTitle.trim().length < 3 || createHomeSlide.isPending || updateHomeSlide.isPending} onClick={() => editingSlideId ? updateHomeSlide.mutate({ id: editingSlideId, ...sliderPayload() }) : createHomeSlide.mutate(sliderPayload())} className="flex-1 rounded-xl bg-[#18344f]">{editingSlideId ? "Güncelle" : "Slider oluştur"}</Button>{editingSlideId && <Button onClick={resetSlideForm} variant="outline" className="rounded-xl">Vazgeç</Button>}</div></div></div><div><div className="flex items-center justify-between"><p className="text-sm font-bold text-[#365368]">Kayıtlı sliderlar</p><span className="text-xs text-[#7b8b90]">{homeSlides.data?.length ?? 0} kayıt</span></div><div className="mt-4 space-y-3">{homeSlides.data?.length ? homeSlides.data.map(slide => <div key={slide.id} className="overflow-hidden rounded-2xl border border-[#e5e8e1] bg-white"><div className="flex gap-3 p-3"><div className="grid h-16 w-16 shrink-0 place-items-center overflow-hidden rounded-xl bg-[#dcebe5] text-xs font-bold text-[#47736a]">{slide.imageUrl ? <img src={slide.imageUrl} alt="" className="h-full w-full object-cover" /> : "SLIDER"}</div><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate text-sm font-bold text-[#29465a]">{slide.title}</p><p className="mt-1 text-xs text-[#77878d]">Sıra {slide.sortOrder} · {slide.isActive ? "Aktif" : "Pasif"}</p></div><Badge variant="outline" className={slide.isActive ? "border-[#b9ded1] text-[#317463]" : "border-[#e2d0c7] text-[#9a6655]"}>{slide.isActive ? "Yayında" : "Pasif"}</Badge></div><div className="mt-3 flex gap-2"><Button onClick={() => editSlide(slide)} size="sm" variant="outline" className="h-8 rounded-lg text-xs">Düzenle</Button><Button onClick={() => deleteHomeSlide.mutate({ id: slide.id })} disabled={deleteHomeSlide.isPending} size="sm" variant="ghost" className="h-8 rounded-lg px-2 text-xs text-[#a05046] hover:bg-[#fff1ed] hover:text-[#a05046]">Sil</Button></div></div></div></div>) : <div className="rounded-2xl border border-dashed border-[#cfd8d1] p-7 text-center text-sm leading-6 text-[#7b8b90]">Henüz slider kaydı yok. İlk kaydı oluşturduğunuzda ana sayfa bu içeriği göstermeye başlayacak.</div>}</div></div></div> : <RestrictedNotice />}</section>}
        ...
        <section className="rounded-[24px] border border-[#e6e6de] bg-white p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-[#71838b]">Kayıtlı kullanıcıların rol ve son giriş bilgilerini yönetin.</p>
            </div>
            <Badge className="border-0 bg-[#edf4ef] text-[#548073]">{adminUsers.data?.length ?? 0} kullanıcı</Badge>
          </div>
          {isAdmin ? <AdminUsersManagement members={adminUsers.data?.map(member => ({ ...member, role: member.role === "user" ? "member" : member.role }))} isLoading={adminUsers.isLoading} isError={adminUsers.isError} isPending={updateUserRole.isPending} onRoleChange={input => new Promise<void>(resolve => { updateUserRole.mutate(input, { onSettled: () => resolve() }); })} /> : <RestrictedNotice />}
        </section>
      )}
